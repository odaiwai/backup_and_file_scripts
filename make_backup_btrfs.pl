...
my $latest_local_backup = return_latest_backup ($local, $prefix)
my $latest_remote_backup = return_latest_backup ($remote, $prefix)
...
if ( -d ("$local/BACKUP") && -d "$remote/BACKUP") {
    ...
    my $result = do_cmd("btrfs subvolume snapshot -r $local $local/BACKUP.$timestamp");
    $result = do_cmd("sync");
    $result = do_cmd("btrfs send -p $local/BACKUP $local/BACKUP.$timestamp | btrfs receive $remote");
    $result = do_cmd("btrfs subvolume list $local");
    if ( (-d "$local/BACKUP") and (-d "$local/BACKUP.$timestamp")) {
        # clean up and increment our backup
        printlog ($log, "3.1.1 Local Backup has old and new versions: cleaning up $local");
        $result = do_cmd("btrfs subvolume delete $local/BACKUP");
        $result = do_cmd("mv $local/BACKUP.$timestamp $local/BACKUP");
    }
    $result = do_cmd("btrfs subvolume list $remote");
    if ( (-d "$remote/BACKUP") and (-d "$remote/BACKUP.$timestamp")) {
