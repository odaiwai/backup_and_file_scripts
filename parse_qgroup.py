...
def pretty_bytes(size, *, scale):
    ...
    longsuffixes = ['Bytes', 'Kibibytes', 'Mebibytes', 'Gibibytes', 'Tebibytes', 'Pebibytes', 'Exbibytes', 'Zebibytes', 'Yobibytes'] # Bytes
        if scale:
        # using powers of 10
        base = 1000
    ...
    for power in range(0,9):
        ...
        if size > divisor:
            pretty_size=sprintf ( '%.2f',(size / divisor)) . '$suffixes[$power]'
